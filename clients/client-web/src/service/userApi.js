...
export async function findUser() {
  ...
  const options = makeOptions(jwtToken);
...
export async function isFavorite(id) {
  ...
  const options = makeOptions(jwtToken);
  const { app_user_id } = jwtDecode(jwtToken);
  const faveCheck = { recipeId: id, userId: app_user_id };
  const response = await axios.post(`${API_URL}/favorite/check`, faveCheck, options);
  if (response.status === 200) {
    return response.data;
  }
...
export async function addFavorite(id) {
  ...
  const options = makeOptions(jwtToken);
  const { app_user_id } = jwtDecode(jwtToken);
  const newFave = { recipeId: id, userId: app_user_id };
...
export async function removeFavorite(id) {
  ...
  const options = makeOptions(jwtToken);
  const { app_user_id } = jwtDecode(jwtToken);
  const byeFave = { recipeId: id, userId: app_user_id };
  const response = await axios.put(`${API_URL}/favorite`, byeFave, options);
  if (response.status === 404) {
